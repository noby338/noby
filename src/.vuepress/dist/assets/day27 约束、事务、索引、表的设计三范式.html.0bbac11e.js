import{_ as n}from"./_plugin-vue_export-helper.cdc0426e.js";import{o as s,c as a,e}from"./app.7ab44a57.js";const t={},i=e(`<h2 id="约束" tabindex="-1"><a class="header-anchor" href="#约束" aria-hidden="true">#</a> 约束</h2><ul><li>定义：限制表中的数据，保证添加到数据表中的数据准确和可靠性！凡是不符合约束的数据，插入时就会失败！ <ul><li>如：性别字段只有男和女，年龄只有正整数；</li></ul></li><li>约束条件在创建表时可以使用， 也可以修改表的时候添加约束条件</li><li>MySQL中的6大约束： <ul><li>默认值约束：可以给字段设置默认值，如果在插入数据时不给该字段赋值，那么就采用默认值</li><li>非空约束：字段的值不能是null(可以没值)</li><li>唯一约束：字段的值是唯一的，该字段不能出现重复的数据</li><li>主键约束：非空+唯一，一般一张表只有一个主键</li><li>自增长约束：新插入的数据的值是在前一条数据的基础上自增而来的</li><li>外键约束：B表中的某个字段的值收到A表某个字段的值的约束</li></ul></li></ul><h3 id="默认值约束" tabindex="-1"><a class="header-anchor" href="#默认值约束" aria-hidden="true">#</a> 默认值约束</h3><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>-- 1、默认值约束：
create table \`user\`(
    id int,
    \`name\` varchar(20),
